...
__global__ void globalMemKernel(int *data) {
	...
	if ( i < N) {
		for (int k = 0; k < LOOP; k++) {
			data[i] = data[i] * 1.111; // read VRAM, add, write VRAM (every loop!)
		}
	}
}
...
__global__ void sharedMemKernel(int *data) {
	...
	if ( i < N) {
		...
		for ( int k = 0; k < LOOP; k++) {
			cache[t] = cache[t] * 1.111; //read shared, add, write shared (fast!)
		}
...
__global__ void registerKernel(int *data) {
	...
	if( i < N) {
		...
		for(int k = 0; k < LOOP; k++) {
			acc = acc * 1.111; // read register, add, write register (0 cycles!)
		}
